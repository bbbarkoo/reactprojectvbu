...
function MyCoursesTable() {
    ...
    let [enrollmentArray, setEnrollmentArray] = useState([]);
    ...
    const fetchData = async () => {
        ...
        const userRef = ref(db, `school/enrolledCourses`);
        const userSnapshot = await get(userRef);

        if (userSnapshot.exists()) {
            const userRecord = userSnapshot.val();

            const tempArray = Object.keys(userRecord).map(myFireId => {
                return {
                    ...userRecord[myFireId],
                    enrollmentId: myFireId
                }
            })

            setEnrollmentArray(tempArray);

            // Fetch the course information for approved records
            tempArray.forEach(async (record) => {
                if (record.approved === "Yes" && record.studentNumber === studentNumber) {
                    const coursesRef = ref(db, `school/courses/${record.courseCode}`);
                    const coursesSnapshot = await get(coursesRef);

                    if (coursesSnapshot.exists()) {
                        setCourseArray(prevCourses => [...prevCourses, coursesSnapshot.val()]);
                    } else {
                                ...
                                <tr tabIndex="0" className="focus:outline-none h-16 border border-gray-100 rounded">
                                    ...
                                    <td>
                                        <div className="flex items-center pl-5">
                                            <p className="text-base font-medium leading-none text-gray-700 mr-2">{course.courseName}
                                            </p>
                                        </div>
                                    </td>
                                    <td className="pl-8">
