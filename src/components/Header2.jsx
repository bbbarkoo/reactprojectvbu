...
function Header2() {
  ...
  } = useContext(StudentContext); const navigate = useNavigate();
  ...
  const getLinkClassName = (path) => {
    return location.pathname === path
      ? "block px-4 py-2 mt-2 text-sm font-semibold text-gray-900 bg-gray-300 rounded-lg focus:text-gray-900 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
      : "block px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline";
  };
      ...
      <header className="border-b-4 border-yellow-400 p-4">
        ...
        <div className="container mx-auto">
          <div className="px-4 flex flex-row justify-between items-center">
            <a href="/landing" className="text-lg font-semibold tracking-widest text-gray-900 uppercase rounded-lg  focus:outline-none focus:shadow-outline">
              VBU Course System
            </a>
          ...
          <nav className={`${isOpen ? 'block' : 'hidden'}`}>
            <a href="/landing" className={getLinkClassName('/landing')}>Home</a>
            <a href="/mycourses" className={getLinkClassName('/mycourses')}>My Courses</a>
            <a href="/courselist" className={getLinkClassName('/courselist')}>Course List</a>
            <a href="/register" className={getLinkClassName('/register')}>Register to Courses</a>
            <button onClick={toggleDropdown} className="block px-4 py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg  focus:outline-none focus:shadow-outline">
              <span className="px-4 py-2 mt-2 text-sm font-semibold text-yellow-500 bg-gray-200 rounded-lg   focus:text-gray-900 focus:bg-gray-200 focus:outline-none focus:shadow-outline">
                ...
                {studentName}
                <svg fill="currentColor" viewBox="0 0 20 20" className={`inline w-4 h-4 ml-1 transition-transform duration-200 transform ${isDropdownOpen ? 'rotate-180' : 'rotate-0'}`}>
                  <path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd"></path>
                </svg>
              </span>
            </button>
            <div className={`${isDropdownOpen ? 'block' : 'hidden'}`}>
              ...
              <div className="px-2 py-2 bg-white rounded-md shadow ">
                <p className="block px-4 py-2 ml-2 text-sm font-semibold bg-gray-200 text-yellow-500 rounded-lg">{studentEmail} - {studentNumber}</p>

                <button onClick={handleSignOut} className="block px-4 py-2 text-sm mt-2 font-semibold bg-transparent rounded-lg  hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline">
                  Sign Out
                </button>
              </div>
            </div>
